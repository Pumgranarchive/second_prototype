...
let get_insert_content_data content_yojson =
  try
    ...
    | _                                   ->
      raise (Pum_exc (return_not_found, "title, summary and text are mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad insert_content format"))
...
let get_update_content_data content_yojson =
  try
    ...
    | Some id, tt, s, tx, td   ->
      ...
      map (fun x -> get_string_list (get_list x)) td
    | _                        ->
      raise (Pum_exc (return_not_found, "content_id is mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad update_content format"))
...
let get_delete_contents_data content_yojson =
  try
    ...
    | _         -> raise (Pum_exc (return_not_found, "contents_id is mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad delete_contents format"))
...
let get_insert_tags_data content_yojson =
  try
    ...
    | _                                         ->
      raise (Pum_exc (return_not_found, "type_name and tags_subjects are mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad insert_tags format"))
...
let get_delete_tags_data content_yojson =
  try
    ...
    match tags_id with
    | Some id   -> get_string_list (get_list id)
    | _         -> raise (Pum_exc (return_not_found, "tags_id is mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad delete_tags format"))
...
let get_insert_links_data content_yojson =
  try
    ...
    | _         ->
      raise (Pum_exc (return_not_found, "id_from, ids_to, tags_id are mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad insert_links format"))
...
let get_update_link_data content_yojson =
  try
    ...
    | _                 ->
      raise (Pum_exc (return_not_found, "link_id and tags_id are mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad update_link format"))
...
let get_delete_links_data content_yojson =
  try
    ...
    | Some id   -> get_string_list (get_list id)
    | _         ->
      raise (Pum_exc (return_not_found, "links_id is mandatory"))
  with
  | Yojson_exc _        ->
    raise (Pum_exc (return_not_found, "Bad delete_links format"))
